...
namespace LimelightHelpers
{
    ...
    inline nt::NetworkTableEntry getLimelightNTTableEntry(std::shared_ptr<nt::NetworkTable> table, const std::string &entryName)
    {
        return table.get()->GetEntry(entryName);
    }
    ...
    inline double getLimelightNTDouble(std::shared_ptr<nt::NetworkTable> table, const std::string &entryName)
    {
        return getLimelightNTTableEntry(table, entryName).GetDouble(0.0);
    }

    inline std::vector<double> getLimelightNTDoubleArray(std::shared_ptr<nt::NetworkTable> table, const std::string &entryName)
    {
        return getLimelightNTTableEntry(table, entryName).GetDoubleArray(std::span<double>{});
    }

    inline std::string getLimelightNTString(std::shared_ptr<nt::NetworkTable> table, const std::string &entryName)
    {
        return getLimelightNTTableEntry(table, entryName).GetString("");
    }

    inline std::vector<std::string> getLimelightNTStringArray(std::shared_ptr<nt::NetworkTable> table,
                                                              const std::string &entryName)
    {
        return getLimelightNTTableEntry(table, entryName).GetStringArray(std::span<std::string>{});
    }

    inline void setLimelightNTDouble(std::shared_ptr<nt::NetworkTable> table, const std::string entryName, double val)
    {
        getLimelightNTTableEntry(table, entryName).SetDouble(val);
    }

    inline void setLimelightNTDoubleArray(std::shared_ptr<nt::NetworkTable> table,
                                          const std::string &entryName,
                                          const std::span<const double> &vals)
    {
        getLimelightNTTableEntry(table, entryName).SetDoubleArray(vals);
    }
    ...
    inline double getLatency_Pipeline(std::shared_ptr<nt::NetworkTable> table)
    {
        return getLimelightNTDouble(table, "tl");
    }
    ...
    inline double getLatency_Capture(std::shared_ptr<nt::NetworkTable> table)
    {
        return getLimelightNTDouble(table, "cl");
    }
    /**
     ...
     */
    inline void setFiducial3DOffset(const std::string &limelightName, double offsetX, double offsetY, double offsetZ)
    {
     ...
     */
    inline void SetFidcuial3DOffset(const std::string &limelightName, double x, double y, double z)
    {
        std::array<double, 3> entries;
        entries[0] = x;
        entries[1] = y;
    ...
    public:
        ...
        // Should these actually use m_TargetXNormalizedCrosshairAdjusted and be part of SingleTargetingResultClass?
        double m_TargetXDegreesNoCrosshairAdjusted{INVALID_TARGET};
        double m_TargetYDegreesNoCrosshairAdjusted{INVALID_TARGET};
    };
